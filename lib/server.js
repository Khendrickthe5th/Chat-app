...
socketIo.on('connection', (socket) => {
    socket.on("join", (user)=>{
        console.log("Request to join group received")
        connectedUsers[socket.id] = user;
        socketIo.emit("userList", connectedUsers)
        socket.join(user)
        socket.on("privateMessage", ({user, message})=>{
            socketIo.to(user).emit("privateMessage", {"sender: ": socket.id, message})
            console.log("privateMessage received! and emitted")
        })
    })
